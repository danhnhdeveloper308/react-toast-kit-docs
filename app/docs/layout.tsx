...
const sidebarSections = [
  ...
  },
  {
    ...
    items: [
      { label: 'DevTools Overview', href: '/docs/devtools' },
      { label: 'Live Playground', href: '/docs/devtools#playground' },
      { label: 'Monitor Tab', href: '/docs/devtools#monitor' },
      { label: 'Create Tab', href: '/docs/devtools#create' },
      { label: 'Settings Tab', href: '/docs/devtools#settings' },
      { label: 'Keyboard Shortcuts', href: '/docs/devtools#shortcuts' },
    ],
  },
  {
    title: 'API Reference',
    ...
    <nav className="space-y-7">
        ...
        <div key={section.title}>
          ...
          <ul className="space-y-0.5">
            {section.items.map((item) => {
              ...
              const isDevTools = section.title === 'Developer Tools';
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                  ...
                  >
                    {isDevTools && (
                      <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                        isActive ? 'bg-violet-500' : 'bg-slate-300 dark:bg-slate-600'
                      }`} />
                    )}
                    {item.label}
                  </Link>
      ...
      <div className="pt-4 border-t border-slate-200 dark:border-slate-800 px-2">
        ...
        <div className="mt-3 flex flex-col gap-1.5">
          ...
          >
            <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M0 0v24h24v-24h-24zm6.89 6.89h10.22v10.22h-3.33v-6.89h-3.33v6.89h-3.56v-10.22zm13.78 13.78h-20.45v-20.45h20.45v20.45z"/></svg>
            NPM Package
          </a>
        </div>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex gap-10">
        ...
        {/* Content */}
        <div className="flex-1 min-w-0 docs-content">
          {children}
        </div>
      </div>
    </div>
