...
import type { ProgressBarStyle, ToastPosition, ToastAnimation, ToastStyle } from 'react-toast-kit';
...
const visualStyles: { style: ToastStyle; label: string; emoji: string }[] = [
  ...
  { style: 'confetti', label: 'Confetti', emoji: '✣' },
];

const progressStyles: ProgressBarStyle[] = [
  'default',
  'fancy',
  'gradient-wave',
  'pulse',
...
export default function ToastDemo() {
  ...
  // --- Variant demos ---
  const showBasic = () =>
    toast({ title: 'Default Toast', description: 'A basic toast notification.', ...opts() });
  const showSuccess = () =>
    toast.success({
      title: 'Success!',
  ...
  const showInfo = () =>
    toast.info({ title: 'Info', description: 'A new version is available.', ...opts() });
  const showLoading = () => {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/80 mt-0.5">
              Fully custom JSX rendered inside the toast.
            </p>
  ...
  const showMultiple = () => {
    const variants = [toast.info, toast.success, toast.warning, toast.error];
    variants.forEach((show, i) => {
      setTimeout(() => {
        show({ title: `Toast #${i + 1}`, description: 'Multiple toast demo', position });
      }, i * 400);
    });
  };
      ...
      actions: [
        {
          label: 'Delete',
          variant: 'danger',
          onClick: () => toast.error({ title: 'Deleted', description: 'File removed.' }),
        },
  ...
  const showConfigured = () =>
    toast.success({
      title: 'Custom config',
      description: 'Using your settings above.',
      ...opts(),
      visualStyle,
    });
        ...
        <div className="p-4 sm:p-6 space-y-6">
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              <button
                onClick={showBasic}
                className="btn-demo bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200"
              >
                Default
              </button>
              <button
                onClick={showSuccess}
                className="btn-demo bg-green-500 hover:bg-green-600 text-white"
              >
                Success
              </button>
              <button
                onClick={showError}
                className="btn-demo bg-red-500 hover:bg-red-600 text-white"
              >
                Error
              </button>
              <button
                onClick={showWarning}
                className="btn-demo bg-amber-500 hover:bg-amber-600 text-white"
              >
                Warning
              </button>
              <button
                onClick={showInfo}
                className="btn-demo bg-blue-500 hover:bg-blue-600 text-white"
              >
                Info
              </button>
              <button
                onClick={showLoading}
                className="btn-demo bg-gray-500 hover:bg-gray-600 text-white"
              >
                Loading → Done
              </button>
            </div>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    toast.info({
                      ...
                      description: 'Toast theme updated.',
                    });
                  }}
        ...
        <div className="p-4 sm:p-6 space-y-6">
              ...
              {progressStyles.map((style) => (
                <button
                  ...
                  onClick={() =>
                    toast.info({
                      title: style,
                      description: 'Inset timeline progress treatment.',
                      position,
                      duration: 5000,
                      progressBarStyle: style,
                    })
                  }
                  className="btn-demo bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 capitalize"
                >
                  {style.replaceAll('-', ' ')}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                onClick={showActions}
                className="btn-demo bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800"
              >
                Confirm / Cancel
              </button>
              <button
                onClick={showActionsSuccess}
                className="btn-demo bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/40 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800"
              >
                View / Copy link
              </button>
            </div>
            <p className="mt-2 text-xs text-gray-400 dark:text-gray-500">
              Action buttons call custom handlers and optionally close the toast.
            </p>
          </div>
              ...
              {animations.map((anim) => (
                <button
                  ...
                  onClick={() =>
                    toast.info({
                      title: anim,
                      description: 'Watch the entrance.',
                      animation: anim,
                      position,
                      duration: 3000,
                    })
        ...
        <div className="p-4 sm:p-6 space-y-6">
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                onClick={showCustom}
                className="btn-demo bg-gradient-to-r from-violet-500 to-purple-500 hover:from-violet-600 hover:to-purple-600 text-white"
              >
                Custom Component
              </button>
              <button
                onClick={showPromise}
                className="btn-demo bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-white"
              >
                Promise Toast
              </button>
              <button
                onClick={showUpdate}
                className="btn-demo bg-gradient-to-r from-orange-400 to-amber-500 hover:from-orange-500 hover:to-amber-600 text-white"
              >
                Updating Toast
              </button>
              <button
                onClick={showMultiple}
                className="btn-demo bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 text-white"
              >
                Multiple Toasts
              </button>
            </div>
              ...
              <button
                onClick={() => {
                  for (let i = 0; i < 3; i++)
                    setTimeout(
                      () =>
                        toast.info({ title: `Toast ${i + 1}`, description: 'Stacked', position }),
                      i * 200
                    );
                }}
  ...
  dismissible: ${dismissible},
          ...
          <button
            onClick={showConfigured}
            className="w-full btn-demo bg-blue-600 hover:bg-blue-700 text-white font-semibold"
          >
